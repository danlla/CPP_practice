...
Protocol::Buffer SendMessage::makeRequest(std::string message)
{
	Buffer buffer;
	writeUnsigned(buffer, (size_t)QueryType::SendMessage);
	writeData(buffer, message);
	Buffer payloadSizeBuffer;
	const auto payloadSize = buffer.size();
	writeUnsigned(payloadSizeBuffer, payloadSize);
...
Protocol::Buffer SendMessage::makeResponse(bool successfuly)
{
	Buffer buffer;

	writeUnsigned(buffer, 2 * UNSIGNED_SIZE);
	writeUnsigned(buffer, (size_t)QueryType::SendMessage);
	writeUnsigned(buffer, (size_t)successfuly);

	return buffer;
}

bool SendMessage::parseResponse(Buffer& buffer)
{
	return (bool)readUnsigned(buffer);
}